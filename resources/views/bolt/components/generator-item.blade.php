...
>
    <div class="px-8 pt-8 pb-5">
        <div class="flex gap-2 justify-between items-center">
            <h4 class="relative mb-3.5 inline-block text-lg font-medium">
                {{ __($item->title) }}
                <span
                    class="inline-block -translate-x-1 align-middle opacity-0 transition-all group-hover:translate-x-0 group-hover:!opacity-100 rtl:-scale-x-100"
                >
            ...
            <x-lqd-icon
                class="lqd-generator-item-icon mb-5 shadow-none shrink-0 bg-transparent group-hover:scale-110"
                size="none"
                style="--color: {{ $item->color }}"
                active-badge
                active-badge-condition="{{ $item->active == 1 }}"
            >
                <span class="size-10 flex items-center justify-center transition-transform group-hover:scale-110" style="color: var(--color)">
                    @if ($item->image !== 'none')
    ...
    <div class="flex gap-y-2 gap-x-3.5 justify-between px-8 border-t border-border py-6 mt-auto">
        <a href="{{ $overlay_link_href }}" class="flex items-center gap-1 text-heading-foreground text-xs">
            @lang('Learn More')
